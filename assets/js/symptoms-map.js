$(document).ready(function () {
  ...
  function draw_map() {
    L.tileLayer("https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png", {
      // attribution: '&copy; <a target="_blank" rel="noopener noreferrer" href="http://cartodb.com/attributions">CartoDB</a>',
      maxZoom: 15,
    }).addTo(map);
  ...
  function update_position() {
    ...
    $.getJSON(
      "https://i2lverrng9.execute-api.eu-west-1.amazonaws.com/dev/symptoms",
      function (data) {
        console.log(data);
        var len = data.count;
  ...
  function dynamic_number(dom, start, finish) {
    ...
    $(dom).prop("number", start).animateNumber(
      {
        number: finish,
